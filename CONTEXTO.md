...
- `averias_huevo(id, lote_huevo_id, referencia_huevo_id, etapa, tipo_averia, cantidad, cantidad_yemas, cantidad_bolsas_yema, orden_cargue_id, fecha, observaciones, usuario_id)` — etapa ∈ {recoleccion, clasificacion, transporte, despacho, recepcion}; tipo_averia ∈ {picado, roto, partido}. `cantidad_yemas`/`cantidad_bolsas_yema` (migración 012) solo se llenan cuando la recepción es multi-línea (bodega destino tipo `venta`); el resto de averías las dejan `null`.
...
- `solicitudes_traslado(id, codigo, bodega_origen_id, bodega_destino_id, estado, usuario_id, creado_en)` — estado ∈ {pendiente, cargado, recibido, anulado}.
- `pedidos(id, codigo, bodega_id, cliente_id, fecha_pedido, fecha_entrega_programada, estado, usuario_id, creado_en)` — **no usada todavía** (Módulo 4).

**Órdenes de cargue (compartidas: traslado y despacho)**
- `ordenes_cargue(id, tipo_operacion, bodega_id, solicitud_traslado_id, pedido_id, orden_cargue_origen_id, codigo, placa_vehiculo, conductor, hora_llegada_vehiculo, hora_inicio_cargue, hora_fin_cargue, hora_inicio_descargue, hora_fin_descargue, peso_total_kg, tarifa_servicio_descargue_id, valor_tarifa_aplicado, estado, usuario_id, creado_en)` — tipo_operacion ∈ {cargue_traslado, descargue_traslado, cargue_despacho}. estado ∈ {pendiente, cargado, en_transito, recibido, cerrado, anulado}. `orden_cargue_origen_id` autorreferencia: la orden de descargue apunta a la orden de cargue que la generó.
