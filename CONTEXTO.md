...
### Ronda 9 — Reorganizar Indicadores por módulo: dashboards de Recolección y Aves (2026-07-17)
El cliente pidió partir la página única `/indicadores` en dashboards dedicados por módulo, con filtros de fecha/galpón/bodega que antes no existían. Requirió 1 migración: `017_indices_indicadores.sql` (solo índices, no cambia datos).

**1. Dashboard de Recolección** (`/recoleccion/indicadores`, nuevo ítem de navegación). Nuevo `lib/indicadores-recoleccion-actions.ts` con 3 funciones (`obtenerIndicadoresRecoleccion`, `obtenerIndicadoresClasificacion`, `obtenerIndicadorCartones`), todas aceptando `FiltrosIndicadores` (fecha inicio/fin, galpón, bodega opcionales). `components/recoleccion/indicadores-recoleccion-view.tsx` centraliza el estado de filtros (compartido entre las 3 pestañas) y los pasa por props a `indicadores-recoleccion-tab.tsx`, `indicadores-clasificacion-tab.tsx` y `indicadores-cartones-tab.tsx`.

**Limitación de datos importante (no es una elección de diseño, es lo que el modelo permite):** las averías de recolección y de clasificación (`averias_huevo.referencia_huevo_id`) siempre se insertan en `null` — nunca quedan atadas a un tipo/talla de huevo. Por eso:
- En Recolección: "cantidad recolectada" sí va por galpón×color (sale de `movimientos_huevo_sin_clasificar.color_id`), pero "buenos/picado/roto/partido" solo va **por galpón** (sin color) — la avería de recolección no tiene ese dato.
- En Clasificación: "cantidad clasificada" sí va por galpón×tipo real (`clasificaciones_detalle.referencia_huevo_id`), pero "buenos/picado/roto/partido" va por **galpón×color** (vía `clasificaciones.color_id`), no por tipo/talla.
- Cartones: "normal" = `clasificaciones.cartones_calculados`; refuerzo/rotos/averiados = `clasificaciones_cartones_extra` agrupado por `motivo`.

**2. Dashboard de Aves** (`/aves/indicadores`, nuevo ítem de navegación). `components/aves/indicadores-aves-view.tsx` — StatChips de galpones activos/inactivos/totales (de `listarGalpones()`) + reutiliza **sin cambios** `obtenerIndicadoresAves()` de `lib/indicadores-actions.ts` (ya calculaba exactamente capacidad/ocupación/mortalidad/sacrificio/edad por galpón y totales desde la Ronda 7). Filtro de fecha nuevo (`components/ui/date-range-filter.tsx`, compartido con el dashboard de Recolección: Desde/Hasta + presets Hoy/Este mes/Este año).
...
### Ronda 10 — Separar Averías de producción y Averías de bodega (2026-07-17)
El cliente observó que `/averias` mezclaba dos cosas conceptualmente distintas: averías del **proceso de producción** (`etapa='recoleccion'`/`'clasificacion'`, ligadas a un galpón) y averías de **logística/bodega** (`etapa='despacho'`/`'recepcion'`, ligadas a un envío, donde vive el flujo de procesamiento de yemas). Eligió manejarlas aparte (dos pantallas de menú, no pestañas dentro de una). No requirió migración — ambas vistas siguen usando la columna `etapa` que ya existía.

**`/averias` (existente, grupo Bodegas y logística) se acotó a bodega**: `lib/averias-actions.ts::listarAverias` gana `.in("etapa", ["despacho","recepcion"])` como filtro base fijo; el `Select` de etapa en `components/averias/averias-view.tsx` se recortó a esas 2 opciones; título pasó a "Averías de bodega". El flujo de procesamiento de yemas, `actualizarEstadoAveria` (usada también desde Historial Diario) y todo lo demás quedó igual — se confirmó que `listarAverias` no tenía otros consumidores antes de acotarla.

**Nueva `/recoleccion/averias` (grupo Recolección) para producción**: nuevo `lib/averias-produccion-actions.ts::listarAveriasProduccion(filtros)` (`.in("etapa", ["recoleccion","clasificacion"])`, join a `lotes_huevo(galpon_id, galpones)` en vez de bodega) y nuevo `components/averias/averias-produccion-view.tsx` — vista de solo lectura (sin checkboxes ni diálogo de yemas, eso no aplica acá), filtrable por galpón y etapa, con `EstadoBadge` mostrando el estado (pendiente/aprobada/rechazada) tal como ya se controla desde Historial Diario.

Verificado con Playwright contra la BD real: `/averias` mostrando solo la única fila real de Recepción existente (con su checkbox correctamente ausente porque ya fue procesada en yemas en una ronda anterior); `/recoleccion/averias` mostrando las 10 averías reales de Recolección/Clasificación con sus badges de estado correctos (incluida la "Rechazada" de la Ronda 8); filtro por galpón G-01 recortando correctamente a 4 filas. Nav con "Averías" en ambos grupos, sin errores de consola. `npx tsc --noEmit` limpio.

### Ronda 11 — Fix de galpón faltante en Indicadores de Recolección + traslados parciales entre galpones (2026-07-17)
Dos observaciones del cliente validadas revisando código antes de tocar nada.

**1. Fix: galpón sin averías desaparecía de la tabla.** En `lib/indicadores-recoleccion-actions.ts::obtenerIndicadoresRecoleccion`, la tabla "buenos/picado/roto/partido por galpón" se armaba únicamente a partir de las filas de `averias_huevo` encontradas — un galpón que recolectó pero tiene 0 averías (o solo rechazadas, que ya no cuentan) nunca generaba una entrada en el mapa y desaparecía de la tabla por completo, en vez de aparecer con sus buenos completos y 0 averías. Se corrigió sembrando `averiasPorGalponMap` con **todos** los galpones que aparecen en `porGalponMap` (cantidad recolectada) antes de superponer las averías reales. Verificado con Playwright: G-02 (400 recolectados, única avería rechazada) ahora aparece con 400 buenos / 0 averías / 0%.

**2. Traslados parciales entre galpones.** Hasta ahora `trasladarLoteAves` (`lib/lotes-aves-actions.ts`) siempre movía el lote de aves completo. Como un `lote_aves` solo puede estar en un galpón a la vez (y `lotes_huevo.lote_aves_id` liga cada cosecha a un único lote), la porción trasladada se convierte en un **lote nuevo** en el galpón destino — decisión confirmada con el usuario: el código de ese lote nuevo lo escribe el usuario (mismo campo que al ingresar un lote), no se autogenera.

`trasladarLoteAves(loteAvesId, galponDestinoId, cantidad, codigoNuevo?, observaciones?)`: si `cantidad === cantidad_actual` del lote, comportamiento idéntico al de siempre (mueve el lote entero, sin pedir código). Si es menor, requiere `codigoNuevo`: descuenta `cantidad_actual` del lote original (se queda en su galpón), inserta un lote nuevo en el galpón destino con `cantidad_ingreso`/`cantidad_actual = cantidad` y **la misma `fecha_ingreso`/`edad_semanas_ingreso` del original** (no reinicia el reloj de edad), y registra el mismo movimiento de tipo `traslado` en `movimientos_aves` **dos veces** (una por cada `lote_aves_id`, porque esa columna es una FK a un solo lote) para que el historial de ambos lotes quede coherente. `components/aves/aves-view.tsx::DialogoTraslado` ganó un input "Cantidad a trasladar" (default = cantidad actual) y un input "Código del nuevo lote" que solo aparece cuando la cantidad es menor al total.

Verificado con Playwright contra la BD real: traslado parcial de 997 aves del lote real `16072026` (G-03, 3.990) a G-01 con código `TEST-PARCIAL-151534` — el original quedó en 2.993 en G-03, el nuevo apareció con 997 en G-01 con la misma edad (40 sem, 16/07/2026), y el historial de ambos lotes mostró el traslado correctamente (el original con todo su historial previo + el nuevo, el lote nuevo con un único evento en vez de vacío). Traslado completo de vuelta (997 = el total del lote nuevo, sin código) confirmado como regresión: se comportó igual que el traslado de siempre, sin crear una fila adicional. `npx tsc --noEmit` limpio.
...
### Ronda 19 — Logo prominente, fusión Lotes/Historial, averías netas

Cuatro ajustes por notas del usuario:

1. **Logo LiPGO con protagonismo.** El PNG es un wordmark **ancho** (798×313, ~2.55:1) que antes se aplastaba en una caja cuadrada y se veía diminuto. `components/logo-marca.tsx` pasó de `size` (cuadrado) a `height` (alto fijo, `width:auto`) para respetar el aspecto real; su fallback es ahora un wordmark de texto "LiPGO". En `sidebar.tsx` el logo va sobre un **chip blanco** (para que el "LiP" oscuro se lea sobre el fondo azul oscuro) con "AVIMOL" en minúsculas espaciadas **debajo** (colapsado: chip mini `height 16`; expandido: `height 30`). En `login-form.tsx` el logo sube a `height 60` sobre el título "AVIMOL". Favicon sigue en `app/layout.tsx`.

2. **Fusión de Lotes de Huevo dentro de Historial diario.** Se eliminó el módulo `/lotes-huevo` (borrados `app/(app)/lotes-huevo/page.tsx` y `components/lotes-huevo/lotes-huevo-view.tsx`; quitada la entrada del nav en `dashboard-data.ts` y su ayuda). Su trazabilidad única (edad al recolectar, lote de aves, bodega, clasificación por referencia/estantería, averías de todas las etapas) se movió al detalle del Historial: la tabla "Lotes de ese día" ganó un botón "Ver trazabilidad" que abre un diálogo anidado alimentado por `obtenerDetalleLoteHuevo(loteHuevoId)` — extendida para devolver también una `cabecera` (edad, lote de aves, bodega). `listarLotesHuevo`/`LoteHuevo` (huérfanos) se eliminaron. La migración 022 limpia los permisos `usuario_modulos` de `/lotes-huevo`.

3. **Tarjeta de averías = cantidad, no líneas.** Los `StatChip` de `components/averias/averias-view.tsx` y `averias-produccion-view.tsx` pasaron de `averias.length` a `averias.reduce((s,a)=>s+a.cantidad,0)` y se renombraron a "Huevos averiados".

4. **Averías netas hacia la clasificadora.** En `lib/recoleccion-actions.ts::registrarRecoleccion`, el saldo de `inventario_huevo_sin_clasificar` ahora se escribe **neto** (total − averías): las averías (que no traen color) se reparten entre los colores recolectados proporcionalmente con mayor-resto (helper `repartirProporcional`). El movimiento `entrada_cosecha` se mantiene en **bruto** (para que el "total recolectado" del Historial y los indicadores no cambien — los tres lectores de `movimientos_huevo_sin_clasificar` filtran por `entrada_cosecha`) y se agrega un movimiento negativo `salida_averia` por color para que el kardex reconcilie con el saldo neto. **Decisión del usuario: "descontar ya, sin reversa"** — rechazar una avería en el Historial ya no devuelve huevos al inventario (el aprobar/rechazar solo afecta reportes). Requiere la migración **022** (agrega `salida_averia` al CHECK de `movimientos_huevo_sin_clasificar.tipo_movimiento`).
